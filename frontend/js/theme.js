...
function applyTheme() {
  document.documentElement.setAttribute('data-theme', getSavedMode());
  document.documentElement.setAttribute('data-color', getSavedColor());
}
...
function openThemePanel() {
  ...
  document.getElementById('themePanelOverlay').style.display = 'flex';
  updatePanelUI();
}

function closeThemePanel() {
  const el = document.getElementById('themePanelOverlay');
  ...
  overlay.innerHTML = `
    <div style="background:var(--card);border:1px solid var(--border);border-radius:24px;
      ...
      <div style="font-family:'Syne',sans-serif;font-size:1.1rem;font-weight:800;
        ...
        color:var(--text);">
        🎨 Appearance
        <button onclick="closeThemePanel()" style="background:var(--surface);
          border:1px solid var(--border);border-radius:8px;padding:5px 12px;
          color:var(--muted);cursor:pointer;font-size:13px;font-weight:600;">✕ Close</button>
      </div>

      <div style="font-size:10px;font-weight:700;letter-spacing:1.5px;
        text-transform:uppercase;color:var(--muted);margin-bottom:8px;">Mode</div>
      <div style="display:flex;background:var(--surface);border:1px solid var(--border);
        ...
        <button id="modeDark" onclick="setMode('dark')" style="flex:1;padding:9px;border:none;
          border-radius:9px;cursor:pointer;font-family:'Syne',sans-serif;font-size:.82rem;
          font-weight:700;">🌙 Dark</button>
        <button id="modeLight" onclick="setMode('light')" style="flex:1;padding:9px;border:none;
          border-radius:9px;cursor:pointer;font-family:'Syne',sans-serif;font-size:.82rem;
          font-weight:700;">☀️ Light</button>
      </div>

      <div style="font-size:10px;font-weight:700;letter-spacing:1.5px;
        text-transform:uppercase;color:var(--muted);margin-bottom:10px;">Color Theme</div>
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:20px;">
        ${Object.entries(THEMES).map(([key, t]) => `
          <div id="sw-${key}" onclick="setColor('${key}')"
            style="height:72px;border-radius:14px;background:${t.grad};cursor:pointer;
              border:3px solid transparent;display:flex;align-items:flex-end;
              justify-content:center;padding-bottom:8px;transition:transform .15s,border-color .15s;
      ...
      <button onclick="closeThemePanel()" style="width:100%;padding:12px;border-radius:12px;
        ...
        transition:opacity .2s;" onmouseover="this.style.opacity='.85'"
        onmouseout="this.style.opacity='1'">Done ✓</button>
    </div>`;
...
function updatePanelUI() {
  ...
  ['modeDark','modeLight'].forEach(id => {
    const btn = document.getElementById(id);
    if (!btn) return;
    const isActive = (id === 'modeDark' && mode === 'dark') || (id === 'modeLight' && mode === 'light');
    btn.style.background = isActive ? 'linear-gradient(135deg,var(--accent),var(--accent2))' : 'transparent';
